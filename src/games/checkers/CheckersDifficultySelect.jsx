...
import { CheckersIcon, ModeRobotIcon } from '../../assets/icons/index.jsx'

const LEVELS = [
  { id: 'easy', label: 'EASY', accent: 'green', tag: 'Single-ply — quick games' },
  { id: 'medium', label: 'MEDIUM', accent: 'cyan', tag: 'Catches simple combos' },
  { id: 'hard', label: 'HARD', accent: 'pink', tag: 'Deeper search — fewer mistakes' },
]
...
export default function CheckersDifficultySelect() {
  ...
  return (
    <div className="relative min-h-screen bg-arcadia-bg text-white">
      ...
      >
        <div className="mx-auto grid max-w-7xl grid-cols-[1fr_auto_1fr] items-center px-4 py-4 md:px-8">
          ...
          <h1 className="inline-flex items-center justify-self-center gap-2 font-arcade text-sm text-neon-green drop-shadow-[0_0_8px_rgba(0,255,136,0.4)] md:text-lg">
            <CheckersIcon size={22} aria-hidden="true" />
      ...
      <main className="mx-auto flex max-w-3xl flex-col items-center px-4 py-12 md:py-20">
        <p className="inline-flex items-center gap-2 font-arcade text-[10px] text-white/45 md:text-xs">
          <ModeRobotIcon size={14} aria-hidden="true" /> SELECT DIFFICULTY
        </p>
        <div className="mt-8 grid w-full grid-cols-1 gap-4 sm:grid-cols-3">
          {LEVELS.map((opt) => (
            <button
              key={opt.id}
              type="button"
              onClick={() => navigate(`/game/checkers/computer/${opt.id}`)}
